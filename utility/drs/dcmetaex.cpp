...
const google::protobuf::FileDescriptor * EXTProtoMeta::LoadFile(const char * file){
    ...
    if (!desc){
		GLOG_ERR("error import file:%s",file);
		return nullptr;
	}
...
google::protobuf::Message	*			 EXTProtoMeta::NewDynMessage(const char * msg_type, const char * buffer, size_t buffer_len){
	...
	if (!msg_desc){
		GLOG_ERR("get msg desc error ! msg_type:%s" ,msg_type);
		return nullptr;
	}
	auto pProtMsg = dyn_msg_factory->GetPrototype(msg_desc);
	if (!pProtMsg){
		GLOG_ERR("get proto msg error ! msg_type:%s", msg_type);
		return nullptr;
	}
	Message * pMsg = pProtMsg->New();
	if (!pMsg){
		GLOG_ERR("new msg from proto error ! msg_type:%s",msg_type);
		return nullptr;
	}
	...
	if (!pMsg->ParseFromArray(buffer, (int)buffer_len)){
		GLOG_ERR("unpack msg error !",msg_type);
		FreeDynMessage(pMsg);
		return nullptr;
	}
	return pMsg;
