...
int     dcshmobj_pool_impl::check_valid() const {
    for (size_t i = 1; i < shm->head.objects_count; ++i){
...
dcshmobj_user_t * dcshmobj_pool_impl::find_user(const char * name){
...
int     dcshmobj_pool_impl::start(const char * keypath){
    ...
    shconf.attach = false;
    bool attached = false;
    int	ret = dcshm_create(shconf, (void **)&shm, attached);
    if (ret){
        GLOG_SER("shm create error alloc size:%zu ret:%d", shconf.shm_size, ret);
        return -1;
    }
    shm_keypath = keypath;
    assert(shm);
    if (attached){
        ...
        //check recover shm
        dcshm_config_t shmbackupconf;
        shmbackupconf.shm_path = shm_keypath;
        shmbackupconf.shm_size = 0;
        shmbackupconf.attach = true;
        shmbackupconf.proj_id = 2;
        dcshmobj_pool_mm_fmt * shmbackup = NULL;
        bool backup_shm_attached = true;
        int	ret = dcshm_create(shmbackupconf, (void **)&shmbackup, backup_shm_attached);
        if (ret){
            GLOG_WAR("not found backup shm error alloc size:%zu ret:%d", ret);
            for (size_t i = 0; i < shm->head.objects_count; ++i){
                const char * objname = shm->head.objects[i].name;
                dcshmobj_user_t * user = find_user(objname);
                assert(user);
...
int     dcshmobj_pool_impl::stop(){
    ...
    //create one
    dcshm_config_t shconf;
    shconf.shm_path = shm_keypath;
    shconf.shm_size = pack_sz;
    shconf.attach = false;
    shconf.proj_id = 2;
    dcshmobj_pool_mm_fmt * backup_shm = NULL;
    bool attached = false;
    int	ret = dcshm_create(shconf, (void**)&backup_shm, attached);
    if (ret){
        GLOG_SER("stopping shm create error alloc size:%zu ret:%d", pack_sz, ret);
        return -1;
    }
    GLOG_WAR("stopping shm create error alloc size:%zu attached:%d", pack_sz, attached?1:0);
